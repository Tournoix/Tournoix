    ...
    match connection.run(
        ...
        Ok(mut nut) => {
            ...
            let bets = match connection.run(
                move |c| bets::table.inner_join(games::table.on(games::id.eq(bets::fk_games))).filter(bets::fk_nuts.eq(nut.id)).filter(games::is_open.eq(true)).select(sum(bets::nb_nut)).first::<Option<i64>>(c)
            ).await {
                Ok(bets) => bets,
                Err(_e) => {
                    return Err((
